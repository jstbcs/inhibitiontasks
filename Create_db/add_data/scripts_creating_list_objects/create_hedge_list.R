# CREATE NESTED LIST OBJECT TO READ IN HEDGE ET AL DATA INTO DB ############################
...
publication_df <- readxl::read_excel("./add_data/Book.xlsx", "publication_table", range = "A4:I4", 
                                     col_names = c("study", "authors", "conducted",
                                                   "added", "country","contact", 
                                                   "keywords", "APA-reference",
                                                   "publication_code")) 
study_df <- readxl::read_excel("./add_data/Book.xlsx", "study_table", range = "A5:D6",
                               col_names = c("study", "n_groups",	"n_tasks", "comment"))
study_df$n_data <- c(2,2) # encode number of data sets per study (by hand)
group_df <- readxl::read_excel("./add_data/Book.xlsx", "group_table", range = "A6:G7",
                               col_names = c("study_in_publication", "study_description",
                                             "between_id",	"mean_age",	"percentage_female",
                                             "n_members",	"group_description"))
task_df <- readxl::read_excel("./add_data/Book.xlsx", "task", range = "A10:D13", 
                              col_names = c("study_within_pub",	"Dataset", "task",
                                            "task_description"))
dataset_df <- readxl::read_excel("./add_data/Book.xlsx", "dataset_overview_table", range = "A10:K13",
                                 col_names = c("study_within_publication", "data",	
                                               ...
                                               "github",	"dataset in R"))
within_df <- readxl::read_excel("./add_data/Book.xlsx", "within_table", range = "A10:D17",
                                col_names = c("study_within_publication",	"data set",
                                              "within_id",	"within_desciption"))
condition_df <- readxl::read_excel("./add_data/Book.xlsx", "condition_descriptives", range = "A14:F21",
                                   col_names = c("study_in_publication",
                                                 "dataset & condition",	"percentage_congr",
                                                 "percentage_neutral",	"mean_obs_pp",	"n_obs"))

# NOTE: read in dataset6, dataset13, dataset49, and dataset50 from "reformat_datasets.R"
...
for(i in 1:nrow(study_df)){ # within each study
  for(k in 1:study_df$n_data[i]){ # within each dataset
...
# save list object -------------------------------------------------------------
saveRDS(pub, file="hedge_list.RData")
