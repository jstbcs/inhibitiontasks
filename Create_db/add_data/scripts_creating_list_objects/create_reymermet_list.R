...
publication_df <- readxl::read_excel("./add_data/Book.xlsx", "publication_table", range = "A3:I3", 
                                     col_names = c("study", "authors", "conducted",
                                                   "added", "country","contact", 
                                                   "keywords", "APA-reference",
                                                   "publication_code")) 
study_df <- readxl::read_excel("./add_data/Book.xlsx", "study_table", range = "A4:D4",
                               col_names = c("study", "n_groups",	"n_tasks", "comment"))
study_df$n_data <- 4 # encode number of data sets per study (by hand)
group_df <- readxl::read_excel("./add_data/Book.xlsx", "group_table", range = "A4:G5",
                               col_names = c("study_in_publication", "study_description",
                                             "between_id",	"mean_age",	"percentage_female",
                                             "n_members",	"group_description"))
task_df <- readxl::read_excel("./add_data/Book.xlsx", "task", range = "A6:D9", 
                              col_names = c("study_within_pub",	"Dataset", "task",
                                            "task_description"))
dataset_df <- readxl::read_excel("./add_data/Book.xlsx", "dataset_overview_table", range = "A6:K9",
                                 col_names = c("study_within_publication", "data",	
                                               ...
                                               "github",	"dataset in R"))
within_df <- readxl::read_excel("./add_data/Book.xlsx", "within_table", range = "A6:D9",
                                col_names = c("study_within_publication",	"data set",
                                              "within_id",	"within_desciption"))
condition_df <- readxl::read_excel("./add_data/Book.xlsx", "condition_descriptives", range = "A6:F9",
...
# save list object -------------------------------------------------------------
saveRDS(pub, file="reymermet_list.RData")
