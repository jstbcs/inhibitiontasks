# CREATE NESTED LIST OBJECT TO READ IN VONBASTIAN ET AL DATA INTO DB ############################
...
publication_df <- readxl::read_excel("./add_data/Book.xlsx", "publication_table", range = "A5:I5", 
                                     col_names = c("study", "authors", "conducted",
                                                   "added", "country","contact", 
                                                   "keywords", "APA-reference",
                                                   "publication_code")) 
study_df <- readxl::read_excel("./add_data/Book.xlsx", "study_table", range = "A7:D7",
                               col_names = c("study", "n_groups",	"n_tasks", "comment"))
study_df$n_data <- 3 # encode number of data sets per study (by hand)
group_df <- readxl::read_excel("./add_data/Book.xlsx", "group_table", range = "A8:G8",
                               col_names = c("study_in_publication", "study_description",
                                             "between_id",	"mean_age",	"percentage_female",
                                             "n_members",	"group_description"))
task_df <- readxl::read_excel("./add_data/Book.xlsx", "task", range = "A14:D16", 
                              col_names = c("study_within_pub",	"Dataset", "task",
                                            "task_description"))
dataset_df <- readxl::read_excel("./add_data/Book.xlsx", "dataset_overview_table", range = "A14:K16",
                                 col_names = c("study_within_publication", "data",	
                                               ...
                                               "github",	"dataset in R"))
within_df <- readxl::read_excel("./add_data/Book.xlsx", "within_table", range = "A18:D20",
                                col_names = c("study_within_publication",	"data set",
                                              "within_id",	"within_desciption"))
condition_df <- readxl::read_excel("./add_data/Book.xlsx", "condition_descriptives", range = "A22:F24",
                                   col_names = c("study_in_publication",
                                                 "dataset & condition",	"percentage_congr",
                                                 "percentage_neutral",	"mean_obs_pp",	"n_obs"))

# NOTE: read in dataset1, dataset7, and dataset10 from "reformat_datasets.R"
...
# save list object -------------------------------------------------------------
saveRDS(pub, file="vonBastian_list.RData")
